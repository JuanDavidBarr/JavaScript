...
export function afterRender(){
    ...
    registerBtn.addEventListener("click", async (event) => {
        ...
        try {
            const responseUsername = await fetch (URL_DB+`users/?username=${username}`);
            const responseEmail = await fetch (URL_DB+`users?email=${email}`);
            ...
            if(!(dataUsername[0] || dataEmail[0])){
                ...
                try {
                    const responseNewUser = await fetch (URL_DB+"users", {
                        method : "POST",
                        headers : {
